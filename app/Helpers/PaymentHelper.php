...
class PaymentHelper
{
    ...
    public function creditReceiverForTransaction($transaction)
    {
        if ($transaction->type != null && $transaction->status == \App\Model\Transaction::APPROVED_STATUS) {
            ...
            switch ($transaction->type) {
                ...
                case \App\Model\Transaction::POST_UNLOCK:
    ...
    private function handleRedirectByTransaction($transaction, $recipient, $message, $success = false)
    {
        ...
        if ($this->isSubscriptionPayment($transaction->type)) {
            ...
            } else if($transaction->payment_provider === \App\Model\Transaction::MANUAL_PROVIDER) {
                $labelType = 'warning';
    ...
    public function generateCoinBaseTransaction($transaction)
    {
        ...
        $coinBaseCheckoutRequest = $httpClient->request('POST', \App\Model\Transaction::COINBASE_API_BASE_PATH . '/charges', [
                'headers' => [
    ...
    {
        $httpClient = new Client();
        $coinBaseCheckoutRequest = $httpClient->request('GET', \App\Model\Transaction::COINBASE_API_BASE_PATH . '/charges/' . $transaction->coinbase_charge_id, [
                'headers' => [
                    'Content-Type' => 'application/json',
                    'X-CC-Api-Key' => getSetting('payments.coinbase_api_key'),
                    'X-CC-Version' => '2018-03-22',
